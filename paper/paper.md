...
2. **Built-in automatic target detection on IFU data.** Unlike most pipelines that rely on post-cube detection, Panacea performs PSF- and fiber-aware automatic detection directly on IFU frames using matched filtering and optimal fiber weighting.
...
The author thanks the Hobby–Eberly Telescope operations staff and the Texas Advanced Computing Center (TACC) for their support of the automated reduction infrastructure. Additional thanks go to HET instrument scientists and LRS2 observers whose feedback guided Panacea’s development and long-term refinement.
