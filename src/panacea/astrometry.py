...
class Astrometry:
    ...
    def __init__(
        ...
        if self.fplane_file is None:
            ...
            self.log.info(
                "Some functions will be unavailable until " "an fplane is given."
            )
            self.fplane = None
        else:
        ...
        self.tp = self.setup_TP(self.ra0, self.dec0, self.rot, self.x0, self.y0)
        self.tp_ifuslot = None

    def setup_TP(self, ra0, dec0, rot, x0=0.0, y0=0.0, x_scale=None, y_scale=None):
    ...
    def set_effective_rotation(self):
        ...
        """
        if self.kind == "fplane":
            self.rot = 360.0 - (90.0 + self.pa + self.sys_rot)
        elif self.kind == "acam":
            self.rot = self.pa + self.sys_rot + 90.0
        elif self.kind == "lrs2":
            self.rot = 180.0 - self.pa - self.sys_rot
        else:
            self.log.error('"kind" was not set to available options.')
            self.log.info("Available options are: %s and %s" % ("fplane", "acam"))
    ...
    def get_ifuslot_projection(self, ifuslot, imscale, crx, cry):
        ...
        self.tp_ifuslot = self.setup_TP(
            ra, dec, self.rot, crx, cry, x_scale=-imscale, y_scale=imscale
        )
...
class IFU(object):
    ...
    def __init__(self, ifuslot, x, y, specid, specslot, ifuid, ifurot, platescl):
...
class FPlane(object):
    ...
    def __init__(
        ...
        self._ifus_by_slot = {}
        self._load_fplane(
            fplane_file, empty_specid, empty_ifuid, exclude_ifuslot, skip_empty
        )
    ...
    def _load_fplane(
        self, fname, empty_specid, empty_ifuid, exclude_ifuslot, skip_empty
    ):
